  ...
  return (
    <DropdownMenuPrimitive.SubTrigger
      ...
      className={cn(
        ...
        isDark
          ? "focus:bg-stone-700 data-[state=open]:bg-stone-700"
          : "focus:bg-stone-100 data-[state=open]:bg-stone-100",
        inset && "pl-8",
        className
      )}
      {...props}
  ...
  return (
    <DropdownMenuPrimitive.Portal>
      <DropdownMenuPrimitive.Content
        ...
        className={cn(
          "z-50 max-h-[var(--radix-dropdown-menu-content-available-height)] min-w-[8rem] overflow-y-auto overflow-x-hidden rounded-md border p-1 shadow-lg",
...
>(({ className, inset, ...props }, ref) => {
  ...
  return (
    <DropdownMenuPrimitive.Item
      ref={ref}
      className={cn(
        "relative flex cursor-default select-none items-center gap-2 rounded-sm px-3 py-2.5 sm:px-2 sm:py-1.5 text-base sm:text-sm outline-none transition-colors data-[disabled]:pointer-events-none data-[disabled]:opacity-50 [&>svg]:size-4 [&>svg]:shrink-0",
        isDark ? "focus:bg-stone-700" : "focus:bg-stone-100",
        inset && "pl-8",
        className
      )}
      {...props}
